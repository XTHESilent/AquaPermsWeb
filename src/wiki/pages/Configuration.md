The main configuration file for AquaPerms can be found at these locations.

| Platform      | Location                                                                                                                      |
|---------------|-------------------------------------------------------------------------------------------------------------------------------|
| Bukkit/Spigot | [`/plugins/AquaPerms/config.yml`](https://github.com/XTHESilent/AquaPerms/blob/master/bukkit/src/main/resources/config.yml)        |
| BungeeCord    | [`/plugins/AquaPerms/config.yml`](https://github.com/XTHESilent/AquaPerms/blob/master/bungee/src/main/resources/config.yml)        |
| Sponge        | [`/config/XTHESilent/AquaPerms.conf`](https://github.com/XTHESilent/AquaPerms/blob/master/sponge/src/main/resources/aquaperms.conf) |
| Fabric        | [`/config/XTHESilent/AquaPerms.conf`](https://github.com/XTHESilent/AquaPerms/blob/master/fabric/src/main/resources/aquaperms.conf) |
| Forge         | [`/config/XTHESilent/AquaPerms.conf`](https://github.com/XTHESilent/AquaPerms/blob/master/forge/src/main/resources/aquaperms.conf)  |
| Nukkit        | [`/plugins/AquaPerms/config.yml`](https://github.com/XTHESilent/AquaPerms/blob/master/nukkit/src/main/resources/config.yml)        |
| Velocity      | [`/plugins/aquaperms/config.yml`](https://github.com/XTHESilent/AquaPerms/blob/master/velocity/src/main/resources/config.yml)      |
...
AquaPerms will attempt to resolve configuration settings from various sources (in the following order).

1. **System Properties**
2. **Environment Variables**
3. **Configuration File** (`config.yml` or `aquaperms.conf`, depending on the platform)
...
e.g. To replicate the following YAML from a AquaPerms config.yml with system properties:
...
```
java
  -Daquaperms.server=example
  -Daquaperms.storage-method=mysql
  -Daquaperms.data.address=192.168.0.100
  -jar server.jar
```
...
e.g. To replicate the following YAML from a AquaPerms config.yml with environment variables:
...
```bash
export AQUAPERMS_SERVER="example"
export AQUAPERMS_STORAGE_METHOD="mysql"
export AQUAPERMS_DATA_ADDRESS="192.168.0.100"
...
* **`database`** - the database which should be used by AquaPerms
...
* **`uuid`** - a cache of `uuid <-- --> username` used by AquaPerms when usernames are used in the `/ap user` command instead of uuids.
* **`log`** - the action log stored by AquaPerms
...
___
### `sync-minutes`
This option controls how frequently AquaPerms will perform a sync task.
...
___
### `watch-files`
When using a file-based storage type, AquaPerms will monitor the data files for changes, and then schedule automatic updates when changes are detected.
...
If enabled and configured, AquaPerms will use the messaging system to inform other connected servers of changes. Use the command "/aquaperms networksync" to push changes. Data is NOT stored using this service. It is only used as a messaging platform.

If you decide to enable this feature, you should set "sync-minutes" to -1, as there is no need for AquaPerms to poll the database for changes.
...
* **`pluginmsg`** - uses the plugin messaging channels to communicate. AquaPerms must be installed on your BungeeCord/Velocity proxy & all connected backend servers. This won't work if you have multiple proxies. The option needs to be set on all LP instances. Using `sql` is recommended over this option!
...
___
### `auto-push-updates`
If AquaPerms should automatically push updates after a change has been made with a command.
...
If you have AquaPerms installed on your backend servers as well as a BungeeCord proxy, you should set this option to false on either your backends or your proxies, to avoid players being messaged twice about log entries.
...
* **`vhost`** - the virtual host to use for AquaPerms. In most cases, this can (and should) be left default. See [here](https://www.rabbitmq.com/vhosts.html) for more information on virtual hosts.
...
How should AquaPerms determine a users "primary" group. The default is `parents-by-weight`.
...
If AquaPerms should run extra permission checks when a player uses commands to modify permission data.
...
Notifications can also be disabled temporarily in-game using `/ap log notify off`
...
How AquaPerms should form prefixes and suffixes.
...
If plugin authors do not provide their own wildcard permissions, then enabling this option will allow AquaPerms to parse them instead. Bukkit especially did not endorse this practice, however it has become common among server administrators. On Sponge, this setting control whether "node.part.\*" style wildcards will function.
...
Plugin authors can define custom permissions structures for their plugin, which will be resolved and used by AquaPerms if this setting is enabled.
...
Plugin authors can define permissions which should be given to all users by default, or setup permissions which should/shouldn't be given to opped players. If this option is set to false, AquaPerms will ignore these defaults.
...
If set to false, AquaPerms will ignore these values.

This is disabled by default, as permissions should really be defined within AquaPerms, so they can be viewed and edited in-game alongside everything else.
...
___
### `auto-op`
If set to true, any user with the permission "aquaperms.autoop" will automatically be granted server operator status.
...
___
### `commands-allow-op`
If opped players should be allowed to use AquaPerms commands.
...
___
### `vault-debug`
If AquaPerms should print debugging info to console when a plugin uses a Vault function
...
If set to true, AquaPerms will allow usernames with non aaphanumeric characters.
...
## contexts.json
The `contexts.json` file is found alongside the main AquaPerms configuration file, and allows you to set two things.
...
AquaPerms provides one static context by default, called "server". This value is set using the `server` option in the main configuration file.
...
Then, I can use `/ap group default permission set some.permission true servertype=factions` to set a permission on all of the factions servers.
...
With the above set, running `/ap user Luck permission set some.permission true` would set `some.permission` to true for me in the `server=survival` context.
