This page shows some sample usages of the AquaPerms API, which is introduced [here](Developer-API).

As well as this documentation, we also have the [api-cookbook](https://github.com/AquaPerms/api-cookbook). This is an example Bukkit plugin which uses the API to perform certain common functions.
...
* [Getting a AquaPerms `User` object](#obtaining-a-user-instance)
...
* [Getting a AquaPerms `Group` or `Track` object](#obtaining-a-grouptrack-instance)
...
A `User` in AquaPerms is simply an object which represents a player on the server, and their associated permission data.

#### Distinction between online & offline players

In order to conserve memory usage, AquaPerms will only load User data when it absolutely needs to. 
...
If we know the player is connected, AquaPerms will already have data in memory for them.

It's as simple as...

```java
Player player = ...;
User user = aquaPerms.getPlayerAdapter(Player.class).getUser(player);
```

Or if you only have a `UUID`...

```java
User user = aquaPerms.getUserManager().getUser(uuid);
```
...
```java
UserManager userManager = aquaPerms.getUserManager();
...
public boolean isAdmin(UUID who) {
    User user = aquaPerms.getUserManager().loadUser(who);
...
public CompletableFuture<Boolean> isAdmin(UUID who) {
    return aquaPerms.getUserManager().loadUser(who)
        .thenApplyAsync(user -> {
            Collection<Group> inheritedGroups = user.getInheritedGroups(user.getQueryOptions());
            return inheritedGroups.stream().anyMatch(g -> g.getName().equals("admin"));
        });
...
```java
Group group = aquaPerms.getGroupManager().getGroup(groupName);
if (group == null) {
    // group doesn't exist.
    return;
}
...
public void addPermission(User user, String permission) {
    ...
    aquaPerms.getUserManager().saveUser(user);
}
```
...
public void addPermission(UUID userUuid, String permission) {
    ...
    aquaPerms.getUserManager().modifyUser(userUuid, user -> {
        // Add the permission
        user.data().add(Node.builder(permission).build());
    });
}
...
The `Node` interface is the core data class in AquaPerms.
...
Contexts are an important concept in AquaPerms, and are introduced [here](Context). They are encapsulated within the API by a few important classes.
...
```java
aquaPerms.getContextManager().registerCalculator(new CustomCalculator());
```
...
ImmutableContextSet contextSet = aquaPerms.getContextManager().getContext(player);
QueryOptions queryOptions = aquaPerms.getContextManager().getQueryOptions(player);
```
...
```java
Optional<ImmutableContextSet> contextSet = aquaPerms.getContextManager().getContext(user);
Optional<QueryOptions> queryOptions = aquaPerms.getContextManager().getQueryOptions(user);
```
...
ContextManager cm = aquaPerms.getContextManager();
...
All `User`s and `Group`s also have an extra object attached to them called `CachedData`. This is the name of the caching class used by AquaPerms to store easily query-able data for all permission holders. The lookup methods provided by this class are very fast. If you're doing frequent data lookups, it is highly recommended that you use `CachedData` over the methods in `User` and `Group`.

Everything in `CachedData` is indexed by `QueryOptions`, as this is how AquaPerms processes all lookups internally.
...
* A AquaPerms `User` or `Group` instance + optionally some `QueryOptions` (see above for how to obtain this)

If you have a `Player` platform instance (like *org.bukkit.entity.Player*), you can use the `PlayerAdapter` to obtain cached data.

```java
Player player = ...;
PlayerAdapter<Player> adapter = aquaperms.getPlayerAdapter(Player.class);

CachedPermissionData permissionData = adapter.getPermissionData(player);
CachedMetaData metaData = adapter.getMetaData(player);
```

If you already have a AquaPerms `User` or `Group` instance, you can use the following methods to obtain cached data.
```java
...
The metadata stored by AquaPerms isn't limited to only a few types. You use the API to easily store any sort of data about players, whilst also taking advantage of the storage / caching systems built into LP.
...
public int getLevel(Player player) {
    ...
    CachedMetaData metaData = aquaPerms.getPlayerAdapter(Player.class).getMetaData(player);
...
AquaPerms uses it's own event system, completely separate from the event systems used by platforms (e.g. Bukkit or Sponge). This means that instead of registering your listener with the server, you must register it directly with AquaPerms.

The events supported by AquaPerms are defined as `interface`s that extend from [`AquaPermsEvent`](https://github.com/XTHESilent/AquaPerms/blob/master/api/src/main/java/net/aquaperms/api/event/AquaPermsEvent.java). They can be found in the [`net.aquaperms.api.event`](https://github.com/XTHESilent/AquaPerms/tree/master/api/src/main/java/net/aquaperms/api/event) package.

#### Event listeners

To listen to events, you first need to obtain the [`EventBus`](https://github.com/XTHESilent/AquaPerms/blob/master/api/src/main/java/net/aquaperms/api/event/EventBus.java) instance using `AquaPerms#getEventBus`, then register each listener using the `subscribe` method.
...
```java
import net.aquaperms.api.event.EventBus;
import net.aquaperms.api.event.log.LogPublishEvent;
import net.aquaperms.api.event.user.UserLoadEvent;
import net.aquaperms.api.event.user.track.UserPromoteEvent;

public class MyListener {
    private final MyPlugin plugin;

    public MyListener(MyPlugin plugin, AquaPerms aquaPerms) {
        this.plugin = plugin;

        EventBus eventBus = aquaPerms.getEventBus();
...
* the [`NodeAddEvent`](https://github.com/XTHESilent/AquaPerms/blob/master/api/src/main/java/net/aquaperms/api/event/node/NodeAddEvent.java) - called when a node is added to a user/group
* the [`NodeRemoveEvent`](https://github.com/XTHESilent/AquaPerms/blob/master/api/src/main/java/net/aquaperms/api/event/node/NodeRemoveEvent.java) - called when a node is removed from a user/group
* the [`NodeClearEvent`](https://github.com/XTHESilent/AquaPerms/blob/master/api/src/main/java/net/aquaperms/api/event/node/NodeClearEvent.java) - called when a user/group has all/some their existing nodes removed

All of these events extend from [`NodeMutateEvent`](https://github.com/XTHESilent/AquaPerms/blob/master/api/src/main/java/net/aquaperms/api/event/node/NodeMutateEvent.java) which defines the base properties.

These events cover all possible changes that could be made to a user/groups AquaPerms data. The trick is to figure out which event you need, and how to filter down to only catch the desired changes.
...
There is an [example listener in the API Cookbook](https://github.com/AquaPerms/api-cookbook/blob/master/src/main/java/me/lucko/apcookbook/listener/PlayerNodeChangeListener.java) which demonstrates this nicely.
