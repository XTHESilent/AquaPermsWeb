...
Firstly, let's clarify how AquaPerms permission storage operates. We get a lot of (sometimes quite angry!) comments from people blaming us for things not working, when 95% of the time it's got nothing to do with AquaPerms. :cry:

In order to write permissions data to a storage backend, AquaPerms uses something called a "driver" to interact with the underlying storage system.
...
* **If the error originates from the driver - it is *not* being caused by AquaPerms.**
...
Some common errors are listed below. If your issue isn't here, then feel free to open a bug report here on GitHub and we can point you in the right direction (or fix the issue if it turns out to be a AquaPerms bug!).
...
> Caused by: java.util.concurrent.CompletionException: java.sql.SQLTransientConnectionException: aquaperms - Connection is not available, request timed out after 5000ms.   
> ...   
> Caused by: java.sql.SQLTransientConnectionException: aquaperms - Connection is not available, request timed out after 5000ms.


> aquaperms - Failed to validate connection com.mysql.jdbc.JDBC4Connection@xxxxxxxxx (Communications link failure)   
...
* MySQL is bound to the correct port, and is accessible from the server where AquaPerms is installed
* Check that your MySQL max connections limit is not being exceeded. By default, AquaPerms will use 10 connections per server. If you have lots of plugins connecting to the same server, you will need to increase this limit.
...
To give the user access to the AquaPerms tables, execute:
```sql
...
For example:
```sql
GRANT ALL PRIVILEGES ON aquaperms.* TO 'luck'@'%';
```
...
You can do this by editing the connection properties in the AquaPerms config file. Under the "Storage" section, locate:
```yaml
data:
  pool-settings:
...
If you get an error similar to:
> me.lucko.aquaperms.lib.hikari.pool.PoolBase - aquaperms-hikari - Failed to validate connection me.lucko.aquaperms.lib.mysql.jdbc.JDBC4Connection@xxxxxxx (No operations allowed after connection closed.)

> me.lucko.aquaperms.lib.hikari.pool.PoolBase - aquaperms-hikari- Failed to validate connection me.lucko.aquaperms.lib.mariadb.MariaDbConnection@xxxxxxx (xxx cannot be called on a closed connection)
...
| `maximum-lifetime` |
|--------------------|
| Location: [AquaPerms config](https://github.com/XTHESilent/AquaPerms/blob/be92a6754404b387dead24ebc1dd3ca8af8e6456/bukkit/src/main/resources/config.yml#L125-L128) |
| Units: milliseconds |
...
Your MySQL user's max connections limit is being exceeded. By default, AquaPerms will use 10 connections per server. If you have lots of plugins connecting to the same MySQL server, you will need to increase this limit.
...
> Caused by: me.lucko.aquaperms.lib.mysql.jdbc.exceptions.jdbc4.MySQLNonTransientConnectionException: Public Key Retrieval is not allowed
...
> Failed to set property port on target class me.lucko.aquaperms.lib.mysql.jdbc.jdbc2.optional.MysqlDataSource
java.lang.NumberFormatException: For input string: xxxx

Your `address` setting in the AquaPerms configuration is unable to set the port, likely due to a mistyped or misformatted address and port. If the port is 3306, as it is by default, there is no need to set the port at all. The correct format for the `address` line is `"address:port"`.

___

### AquaPerms cannot connect to my Redis server
Check that the following is correct:
